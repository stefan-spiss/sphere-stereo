...
__global__ void reprojectDistanceKernel(const float* distanceIn, float* distanceOut,
    ...
    if (indexIn < COLS * ROWS) {
        ...
        if (!isnan(pt.x) && !isnan(pt.y) && !isnan(pt.z)) {
            ...
            float2 outPx = camModel.project(pt, *calib);
            ...
            if (distance < distanceOut[indexOut]) distanceOut[indexOut] = distance;
        }
    }
}
...
__global__ void createInpaintingWeightsKernel(uchar2* inpaintDirWeights, const Intrinsics_CV* calib,
    ...
    if (indexIn < COLS * ROWS) {
        ...
        uchar valx = ((uchar)(weights[0] * 255.f) & 240) + 4 * uchar(1 + neighbours[0].y) +
                     uchar(1 + neighbours[0].x);
        uchar valy = ((uchar)(weights[1] * 255.f) & 240) + 4 * uchar(1 + neighbours[1].y) +
                     uchar(1 + neighbours[1].x);
...
extern "C" __global__ void inpaintKernel(float* distanceMap, const uchar2* inpaintDirWeights) {
    ...
    if (indexIn < COLS * ROWS && distanceMap[indexIn] >= MAX_DIST + 0.1) {
        ...
        if (currentPixel.x < COLS - 1 && currentPixel.y < ROWS - 1 && currentPixel.y >= 1 &&
            currentPixel.x >= 1) {
            ...
            // Fill the current pixel if at least one of the neighbours in the inpainting table has
            // a meaningful value
...
__global__ void mergeRGBDPanoramaKernel(const float2* samplingLut, const float* blendingWeights,
    ...
    if (indexIn < PANO_ROWS * PANO_COLS) {
        ...
        for (int referenceIndex = 0; referenceIndex < REFERENCES_COUNT; referenceIndex++) {
            ...
            // Find the corresponding pixel
            float2 rgbUV = camModel.project(pt, calibs[referenceIndex]);

            // Scale as the stitching colour images may have a different resolution
            float2 stitchingCalibRatio = {float(stitchingImgsCols) / COLS,
                                          float(stitchingImgsRows) / ROWS};
